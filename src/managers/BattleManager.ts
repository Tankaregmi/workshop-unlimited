...
import SocketM from './SocketManager';
...
class BattleManager
{
  static async resolveAction (battle: Battle, action: ActionType, _args: BattleActionArguments = {}) {
    ...
    if (battle.multiplayer) {
      try {
        await SocketM.battle_action({ args, action });
      } catch (error) {
        console.warn(error);
        return;
      }
  ...
  static passTurn (battle: Battle): void {
    ...
    if (attacker.stats.health < 1 || defender.stats.health < 1) {
      ...
      const p1 = attacker.battle.players[0];
      const p2 = attacker.battle.players[1];

      attacker.battle.over = true;
      attacker.battle.victory = p1.stats.health > p2.stats.health;
      ...
      if (!attacker.battle.multiplayer) {
        this.tickAI(attacker.battle);
      }
    };
