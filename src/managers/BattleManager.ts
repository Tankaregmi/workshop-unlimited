...
const eventHandlers = {
	...
	stomp (battle: Battle, event: BattleEvent): void {
		...
		const { attacker } = BattleUtils.getAttackerAndDefender(battle);
    const legsIndex = 1;
    const legs = attacker.items[1] as Item;
    ...
    const item = attacker.items[weaponIndex] as Item;
...
function log (battle: Battle, type: number, ...msg: any): void {
	...
	const color = battle.turnOwnerID === battle.player.id ? '#77ff77' : '#ff7777';
	...
	battle.logs.push({ color, message: `[${typeSufixes[type]}] ${message}` });
...
function passTurn (battle: Battle): void {
	...
	if (attacker.stats.health <= 0 || defender.stats.health <= 0) {
		setBattleComplete(battle, battle.player.stats.health > 0);
...
function setBattleComplete (
	battle: Battle,
	victory: boolean = false,
	quit: boolean = false
): void {
	battle.complete = { victory, quit };
}
